...
/*
  ...
  */
  ...
  if(Qti_esquerda > 800 && Qti_direita <= 800){
    
ir_esquerda();
  
}else if(Qti_direita > 800 && Qti_esquerda <= 800){
...
int line_timer(int tempo){
  ...
  if(Qti_direita > 800 && Qti_esquerda > 800){
    sair == true;
    ativar = 1;
  }
...
int liner(){
      ...
      Qti_esquerda = analogRead(pin_Qti_esquerda);
      Qti_direita = analogRead(pin_Qti_direita);  
      }while(Qti_direita > 800 && Qti_esquerda > 800);
      line_align_rotation();
      if(line_timer(750) == 1){
        return 1;
      }else{
