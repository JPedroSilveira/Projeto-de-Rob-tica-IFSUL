...
/*
  ...
  */
  ...
  if(Qti_esquerda > 850 && Qti_direita <= 850){
    
ir_esquerda();
  
}else if(Qti_direita > 850 && Qti_esquerda <= 850){
...
int line_timer(int tempo){
  ...
  if(Qti_direita > 850 && Qti_esquerda > 850){
    sair = true;
    ativar = 1;
  }
...
int liner(){
      ...
      Qti_esquerda = analogRead(pin_Qti_esquerda);
      Qti_direita = analogRead(pin_Qti_direita);  
      }while(Qti_direita > 850 || Qti_esquerda > 850);
      line_align_rotation();

        int a = line_timer(750);
